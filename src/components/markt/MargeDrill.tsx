...
interface MedewerkerRij { id: number; naam: string; bedrijf: string; bron: TariefBron; tarief: number; urenSel: number; omzet: number; kosten: number; profiel: UrenProfiel }
type SortKey = 'omzet' | 'marge' | 'urenSel' | 'declAlle' | 'naam'
...
export function MargeDrill({ ent, seg, metMh, maanden, maandTabel }: Props) {
  ...
  const medewerkers = useMemo<{ rijen: MedewerkerRij[]; nietToerekenbaar: number }>(() => {
    ...
    const rij = (m: DetailMedewerker) => {
      let r = map.get(m.id)
      if (!r) { r = { id: m.id, naam: m.naam, bedrijf: m.bedrijf, bron: m.bron, tarief: m.tarief, urenSel: 0, omzet: 0, kosten: 0, profiel: urenProfiel(m, maanden) }; map.set(m.id, r) }
      return r
    }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div>
            ...
            <table style={{ borderCollapse: 'collapse', fontSize: 10.5, width: '100%' }}>
              <thead>
                <tr style={{ color: 'var(--t3)' }}>
                  ...
                  {sortTh('urenSel', 'Uren sel.', 'Productieve uren op de projecten in deze selectie')}
                  ...
                  {sortTh('marge', 'Marge')}
                  <th style={th()}>%</th>
                  <th style={th('right', { borderLeft: '1px solid var(--bd2)' })} title="Alle geschreven uren van deze medewerker in de periode (alle projecten, incl. verlof/ziekte)">Uren tot.</th>
                  ...
                  {sortTh('declAlle', 'Decl. alle', 'Declarabiliteit = klanturen / alle geschreven uren (incl. verlof, ziekte)')}
                  <th style={th()} title="Declarabiliteit excl. verlof/ziekte/bijz. verlof (definitie Power BI)">Decl. excl.</th>
                </tr>
              </thead>
              <tbody>
                {zichtbaar.map(r => {
        ...
        <div style={{ fontSize: 10.5, color: 'var(--t3)', marginTop: 6, lineHeight: 1.5 }}>
          Omzet en marge per medewerker zijn toegerekend: per project krijgt iedereen zijn aandeel in de projecturen × de projectomzet
...
function MedewerkerDetail({ m, mod, metMh, maanden, periodeLabel, inSelectie, onClose }: {
  ...
  const rows = mod.detailProjecten.flatMap(p => {
    ...
    return [{ p, u, share, omzet: share * k.omzet, kosten: somM(e.kosten) + share * k.mhKosten }]
  }).sort((a, b) => b.u - a.u)
  const klanten = new Map<string, { n: number; u: number; omzet: number; kosten: number }>()
  for (const r of rows) {
    const key = r.p.intern ? '(intern)' : r.p.klant || '(geen klant)'
    const c = klanten.get(key) ?? { n: 0, u: 0, omzet: 0, kosten: 0 }
    c.n++; c.u += r.u; c.omzet += r.omzet; c.kosten += r.kosten; klanten.set(key, c)
  }
  const tot = rows.reduce((a, r) => ({ u: a.u + r.u, omzet: a.omzet + r.omzet, kosten: a.kosten + r.kosten }), { u: 0, omzet: 0, kosten: 0 })
  ...
  return (
    <div style={{ border: '1px solid var(--bd3)', borderRadius: 8, padding: '10px 12px', marginBottom: 12, background: 'var(--bg1)' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, flexWrap: 'wrap', marginBottom: 8, fontSize: 11.5, color: 'var(--t2)' }}>
        ...
        <span>Omzet <b style={{ color: 'var(--t1)', fontFamily: 'var(--mono)' }}>{fmtEur(tot.omzet)}</b> · marge <b style={{ color: margeColor(tot.omzet - tot.kosten), fontFamily: 'var(--mono)' }}>{fmtEur(tot.omzet - tot.kosten)}</b> over alle projecten</span>
      ...
      <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'flex-start' }}>
        <table style={{ borderCollapse: 'collapse', fontSize: 10.5, flex: 2, minWidth: 520 }}>
          <thead>
            <tr style={{ color: 'var(--t3)' }}>
              <th style={th('left')}>Project</th><th style={th('left')}>Klant</th><th style={th('left')}>Entiteit · segment</th><th style={th()}>Uren</th><th style={th()} title="Aandeel van deze medewerker in de projecturen">Aandeel</th><th style={th()}>Omzet</th><th style={th()}>Kosten</th><th style={th()}>Marge</th><th style={th()}>%</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(r => {
              ...
              return (
                <tr key={r.p.id} style={{ borderTop: '1px solid var(--bd2)', color: sel ? 'var(--t1)' : 'var(--t2)' }}>
                  ...
                  <td style={td('right')}>{Math.round(r.share * 100)}%</td>
            ...
            <tr style={{ borderTop: '2px solid var(--bd3)', color: 'var(--t1)', fontWeight: 700 }}>
              ...
              <td style={td('right', { fontFamily: 'var(--mono)' })}>{fmtU(tot.u)}</td><td />
              <td style={td('right', { fontFamily: 'var(--mono)' })}>{fmtK(tot.omzet)}</td>
              <td style={td('right', { fontFamily: 'var(--mono)' })}>{fmtK(tot.kosten)}</td>
              <td style={td('right', { fontFamily: 'var(--mono)', color: margeColor(tot.omzet - tot.kosten) })}>{fmtK(tot.omzet - tot.kosten)}</td>
              <td style={td('right')}>{pctStr(tot.omzet - tot.kosten, tot.omzet)}</td>
        ...
        <table style={{ borderCollapse: 'collapse', fontSize: 10.5, flex: 1, minWidth: 300 }}>
          <thead>
            <tr style={{ color: 'var(--t3)' }}>
              <th style={th('left')}>Klant</th><th style={th()}>Proj.</th><th style={th()}>Uren</th><th style={th()}>Omzet</th><th style={th()}>Marge</th>
            </tr>
          </thead>
          <tbody>
            {[...klanten.entries()].sort((a, b) => b[1].u - a[1].u).map(([k, c]) => (
              <tr key={k} style={{ borderTop: '1px solid var(--bd2)', color: 'var(--t1)' }}>
                ...
                <td style={td('right', { fontFamily: 'var(--mono)' })}>{k === '(intern)' ? '—' : fmtK(c.omzet)}</td>
      ...
      <div style={{ fontSize: 10.5, color: 'var(--t3)', marginTop: 6 }}>● = project valt in de huidige tegel/klant/project-selectie. Interne projecten tellen kosten maar geen omzet.</div>
    </div>
  )
}
